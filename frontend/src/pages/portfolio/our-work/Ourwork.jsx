  ...
  return (
    <>
      ...
      <PageLayout className="py-24 space-y-16">
        <Headline title="Our Projects" description="A showcase of innovative solutions delivered across industries." />

        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {workData.map((project, index) => (
            <ProjectCard key={index} project={project} index={index} shouldReduceMotion={shouldReduceMotion} />
          ))}
        </div>
        ...
        <div className="space-y-24">
          {workData.map((project, index) => (
            <DetailedProject key={index} project={project} index={index} shouldReduceMotion={shouldReduceMotion} />
          ))}
        </div>
      </PageLayout>
    </>
  ...
  return (
    ...
    >
      ...
      <div className="p-6 text-left">
        <TypographyH4 className="text-gray-900 dark:text-gray-100 mb-2">{project.title}</TypographyH4>
        <CardDescription className="text-gray-600 dark:text-gray-300 text-sm mb-4">{project.description}</CardDescription>
        <div className="flex flex-wrap gap-2">
          {(project.technologies || "React, Node.js, Tailwind CSS").split(", ").map((tech, i) => (
            <motion.span
              key={i}
  ...
  return (
    ...
    >
      ...
      <div className="md:w-1/2 text-left">
        <TypographyH3 className="text-teal-500 mb-4">{project.title}</TypographyH3>
        <p className="text-gray-700 dark:text-gray-300 mb-4">{project.description}</p>

        <div className="mb-4">
          <TypographyH4 className="mb-2">Technologies & Tools</TypographyH4>
          <p className="text-gray-600 dark:text-gray-300">{project.technologies}</p>
        </div>

        <div className="mb-4">
          <TypographyH4 className="mb-2">Methodologies & Practices</TypographyH4>
