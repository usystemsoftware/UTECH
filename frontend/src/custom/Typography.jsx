...
export function Headline({ title, description, className }) {
  return (
    <FadeInWhenVisible>
      <div
        className={cn(
          "space-y-4 w-full sm:max-w-2xl md:max-w-4xl lg:max-w-[80%] mx-auto text-center",
          className
        )}
      >
...
export function TypographyLead({ children, className }) {
  return (
    <p className={cn("text-muted-foreground text-base sm:text-md", className)}>
      {children}
    </p>
  );
}
...
export function HeadSkipper({ children, className }) {
  return (
    <div className="flex items-center gap-2 sm:gap-4 mb-6">
      <h2
        className={cn(
          "text-primary sm:text-2xl text-lg font-semibold tracking-tight uppercase",
          className
        )}
      >
        {children}
